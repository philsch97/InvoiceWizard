...
public partial class CustomerHandling : Page
{
    ...
    {
        var customerName = (NewCustomerNameText.Text ?? "").Trim();
        if (string.IsNullOrWhiteSpace(customerName))
        {
            SetStatus("Bitte einen Kundennamen eingeben.", StatusMessageType.Error);
            return;
        }
        ...
        var saved = await App.Api.SaveCustomerAsync(customerName, defaultMarkup);
    ...
    private async void AddProject_Click(object sender, RoutedEventArgs e)
    {
        ...
        var projectName = (NewProjectNameText.Text ?? "").Trim();
    ...
    private void AllocationsGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        if (AllocationsGrid.SelectedItem is LineAllocationEntity allocation)
        {
            ...
            CustomerInvoiceNumberText.Text = allocation.CustomerInvoiceNumber ?? "";
        }
    }
    ...
    private void WorkEntriesGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        {
            CustomerInvoiceNumberText.Text = workEntry.CustomerInvoiceNumber ?? "";
        }
    }
    ...
    private async void MarkSelectedAsInvoiced_Click(object sender, RoutedEventArgs e)
    {
        ...
        var invoiceNumber = (CustomerInvoiceNumberText.Text ?? "").Trim();
    ...
    private async void ExportOpenItems_Click(object sender, RoutedEventArgs e)
    {
        ...
        var allocations = (await App.Api.GetAllocationsAsync(customer.CustomerId, selectedProjectId)).Where(a => string.IsNullOrWhiteSpace(a.CustomerInvoiceNumber)).OrderBy(a => a.InvoiceLine.Invoice.InvoiceDate).ThenBy(a => a.InvoiceLine.Position).ToList();
        var workEntries = (await App.Api.GetWorkTimeEntriesAsync(customer.CustomerId, selectedProjectId)).Where(w => string.IsNullOrWhiteSpace(w.CustomerInvoiceNumber)).OrderBy(w => w.WorkDate).ThenBy(w => w.StartTime).ToList();
        ...
        var projectSuffix = selectedProjectId.HasValue ? $"_{selectedProjectName}" : "";
        var saveDialog = new SaveFileDialog { Filter = "Excel Arbeitsmappe (*.xlsx)|*.xlsx", FileName = $"{customer.Name}{projectSuffix}_offene_positionen_{DateTime.Now:yyyyMMdd}.xlsx" };
    ...
    private async Task LoadCustomersAsync(string? selectCustomerName = null, int? selectedProjectId = null)
    {
        ...
        if (CustomerCombo.SelectedItem is CustomerEntity customer)
        {
            await LoadProjectsAsync(customer, null, selectedProjectId);
            await LoadCustomerDataAsync();
        }
    }
    ...
    private async Task LoadCustomerDataAsync()
    {
        ...
        DefaultMarkupText.Text = customer.DefaultMarkupPercent.ToString("0.##", CultureInfo.GetCultureInfo("de-DE"));
